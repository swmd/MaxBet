    ...
    async.waterfall([
      ...
      function(connection,callback) {
        rethinkdb.db('polls').tableCreate('poll').run(connection,function(err,result) {
          connection.close();
          if(err) {
            console.log("Poll table already created");
          } else {
            console.log("Created new polls table");
          }
          callback(null,"Database is setup successfully");
        });

        rethinkdb.db('polls').tableCreate('games').run(connection,function(err,result) {
          connection.close();
          if(err) {
            console.log("Games table already created");
          } else {
            console.log("Created new games table");
          }
          callback(null,"Database is setup successfully");
        });
